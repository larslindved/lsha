...
def parse_data(path: str):
    ...
    with open(path) as csv_file:
        ...
        for i, row in enumerate(reader):
            if i == 0:
                dummy_ts = Timestamp(0, 0, 0, 0, 0, 0)
                power.points.append(SignalPoint(dummy_ts, -9999.9))
                speed.points.append(SignalPoint(dummy_ts, -9999.9))
                pressure.points.append(SignalPoint(dummy_ts, -9999.9))
                continue
            ...
            if i > 1 and ts == speed.points[-1].timestamp:
                ...
                # pressure_v = row[1]
                # pressure.points[-1].value = pressure_v
                ...
                # print(f"row: {row}\npressure_v: {pressure_v}\ntype(pressure_v): {type(pressure_v)}\n\n")
        # raise SystemExit()
